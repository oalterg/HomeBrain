...
def backup_config():
    ...
    try:
        ...
        # Migration Safety: Ensure legacy file is removed to prevent double backups
        if os.path.exists(LEGACY_BACKUP_CRON_FILE):
            try:
                os.remove(LEGACY_BACKUP_CRON_FILE)
                logging.info("Removed legacy backup cron file during config save.")
            except Exception as e:
                logging.warning(f"Failed to cleanup legacy cron file: {e}")
...
def set_ai_model():
    ...
    try:
        ...
        update_env_var("AI_MODEL_REASONING", str(model.get("reasoning", False)).lower())
